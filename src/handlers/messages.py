...
async def handle_photo(message: Message):
    ...
    if not user or user.credits < generation_cost:
        ...
        await bot.send_message(
            chat_id,
            messages.INSUFFICIENT_CREDITS.format(credits_balance=credits_balance)
        )
        return
    ...
    try:
        ...
        await bot.send_message(
            chat_id,
            messages.PROMPT_GENERATION_ERROR
        )
        return
    ...
    try:
        ...
        await bot.send_message(
            chat_id,
            messages.IMAGE_GENERATION_SUBMISSION_ERROR
        )
        return

    await bot.send_message(
        chat_id,
        messages.REQUEST_RECEIVED.format(gen_uid=gen_uid),
        parse_mode="Markdown"
    )
